...
class TipCourbeDeux extends StatefulWidget {
  const TipCourbeDeux({Key? key}) : super(key: key);

  @override
  State<TipCourbeDeux> createState() => _TipCourbeDeuxState();
}
...
class _TipCourbeDeuxState extends State<TipCourbeDeux> {
  ...
  List<ChartSeries<ChartDataCourbes, int>> _socTension(List<ChartDataCourbes> dats) { //Liste de la partie intensité/temps

    return <ChartSeries<ChartDataCourbes, int>>[
      SplineAreaSeries(
        animationDelay: 20000,
        dataSource: dats,
        xValueMapper: (ChartDataCourbes data, _) => data.tension,
        yValueMapper: (ChartDataCourbes data, _) => data.valSoC,
                  ...
                  //on affiche la seconde courbe après les :
                    child: SfCartesianChart( //Courbe qui s'affiche lors de l'appui du boutton
                        ...
                        primaryXAxis: NumericAxis(
                          ...
                          maximum: (snapshot.data!.last.tension).toDouble(),   //valeur maximale de l'axe des axis aux regards des données
                        ...
                        series: _socTension(snapshot.data!)
